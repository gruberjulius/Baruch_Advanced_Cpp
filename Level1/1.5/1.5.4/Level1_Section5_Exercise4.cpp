...
struct Finder
{
	static int best_index(const Vec &v, X_t x)
	{
		...
		{
			// as the vector is sorted, we know that if the next value of
			// the vector is bigger than x, then we found the interval
			for (int i{1}; i < N; ++i)
			{
				if (v[i] > x)
				{
...
struct CompareVector
{
	static std::tuple<T, int> max_error(const Vec &v1, const Vec &v2, int i, int j, ErrorTypes errortype)
	{
		if ((i < 0) || (i > j) || (j > N))
		{
			std::cout << "i and j must be in range, i bigger than j " << std::endl;
